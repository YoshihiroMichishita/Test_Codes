...
void H_mom(parm parm_, double k[D], Complex H[M*M]){
    ...
    double sz = parm_.mu + parm_.t_i * s(k[0]);
    Complex sx = parm_.t_i * s(k[1]) + I*parm_.NH;
    double ee = parm_.Ma + parm_.t_e * (c(k[0])+c(k[1]));
    H[0] = sz + ee; H[1] = sx; H[2] = sx; H[3] = sz - ee;

}

void H_mom_BI(parm parm_, double k[D], Complex H[M*M], Complex VR_k[M*M], Complex VL_b[M*M], double E[M]){
    double sz = parm_.mu + parm_.t_i * s(k[0]);
    double sx = parm_.t_i * s(k[1]);
    double ee = parm_.Ma + parm_.t_e * (c(k[0])+c(k[1]));
    H[0] = sz + ee; H[1] = sx; H[2] = sx; H[3] = sz - ee;
...
void H_mom_NH(parm parm_, double k[D], Complex H[M*M], Complex VR_k[M*M], Complex VR_b[M*M], Complex VL_k[M*M],Complex VL_b[M*M],Complex E_NH[M]){
    ...
    Complex sz = parm_.mu + parm_.t_i * s(k[0]);
    Complex sx = parm_.t_i * s(k[1]) + I*parm_.NH;
    double ee = parm_.Ma + parm_.t_e * (c(k[0])+c(k[1]));
...
void Vx(parm parm_, double k[D], Complex H[M*M]){
    ...
    /*
    double sz = parm_.t_i * c(k[0]);
    double sx = 0;
    double ee = -parm_.t_e * s(k[0]);
    H[0] = sz + ee; H[1] = sx; H[2] = sx; H[3] = -sz + ee;*/
...
void Vxx(parm parm_, double k[D], Complex H[M*M]){
    ...
    /*
    double sz = -parm_.t_i * s(k[0]);
    double sx = 0;
    double ee = -parm_.t_e * c(k[0]);
    H[0] = sz + ee; H[1] = sx; H[2] = sx; H[3] = -sz + ee;*/
...
void Vy(parm parm_, double k[D],Complex H[M*M]){
    /*
    double sz = 0;
    double sx = parm_.t_i * c(k[1]);
    double ee = -parm_.t_e * s(k[1]);
    H[0] = sz + ee; H[1] = sx; H[2] = sx; H[3] = -sz + ee;*/
...
void Vz(parm parm_, double k[D], Complex H[M*M]){
    double s_x = 0;
    double s_y = 0;
    double s_z = -parm_.t_i * s(k[2]);
    double s_0 = parm_.t_e * c(k[2]);
...
Ham::Ham(parm parm_,double k[D],int sw){
    ...
    //calculate with the Non-Hermitian band index
        ...
        //Vyx(parm_,k,VYX);
        //Vyxx(parm_,k,VYXX);
        NH_factor(VX,VY,VXX,VR_k,VR_b,VL_k,VL_b,VX_LL,VX_RR,VX_LR,VY_LL,VY_RR,VY_LR,VXX_LL,VXX_LR,NH_fac);
    }
    else if(sw==3){
        H_mom_NH(parm_,k,H_k,VR_k,VR_b,VL_k,VL_b,E_NH);
        H_mom_BI(parm_,k,H_k,VR_k,VL_b,EN);
